...
#include "PeakFinder1D.hpp"

int main(int argc, const char * argv[]) {
    std::vector<int> ints0;
    auto p0 = FindAPeak(ints0.cbegin(), ints0.cend());
    if (p0 != ints0.cend())
        std::cout << "0 found " << *p0 << "\n";
    else
    ...
    auto p1 = FindAPeak(ints1.cbegin(), ints1.cend());
    if (p1 != ints1.cend())
        std::cout << "1 found " << *p1 << "\n";
    else
    ...
    auto p2 = FindAPeak(ints2.cbegin(), ints2.cend());
    if (p2 != ints2.cend())
        std::cout << "2 found " << *p2 << "\n";
    else
    ...
    auto p3 = FindAPeak(ints3.cbegin(), ints3.cend());
    if (p3 != ints3.cend())
        std::cout << "3 found " << *p3 << "\n";
    else
    ...
    auto p4 = FindAPeak(ints4.cbegin(), ints4.cend());
    if (p4 != ints4.cend())
        std::cout << "4 found " << *p4 << "\n";
    else
